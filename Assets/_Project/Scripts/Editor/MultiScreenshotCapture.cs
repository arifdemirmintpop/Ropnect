...
namespace MultiScreenshotCaptureNamespace
{
	...
	public class MultiScreenshotCapture : EditorWindow
	{
        ...
        void Update()
        {
			if (Input.GetKeyDown(KeyCode.Space))
			{
				...
				if (!File.Exists(filePath))
				{
					...
					{
						writer.WriteLine("The screenshots taken should be shared on the game's slack channel. They will also be used for the Play Store.");
					}
				}

				CaptureScreenshot(new Vector2(1080, 1920));
				CaptureScreenshot(new Vector2(1290, 2796));
				CaptureScreenshot(new Vector2(1242, 2688));
				CaptureScreenshot(new Vector2(2048, 2732));
		...
		private List<Vector2> resolutions = new List<Vector2>() {  }; // Not readonly to support serialization
		private List<bool> resolutionsEnabled = new List<bool>() { false }; // Same as above
		...
		{
			string folderName = width.ToString() + "x" + height.ToString();
			...
			do
			{
				path = Path.Combine( folderPath, string.Format( filename, ++fileIndex ) );
